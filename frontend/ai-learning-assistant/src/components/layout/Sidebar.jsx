...
const Sidebar = ({ isSidebarOpen, toggleSidebar }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-full w-64 bg-white/90 backdrop-blur-lg border-r border-slate-200 z-50 transform transition-transform duration-300 ${
      ...
      >
        ...
        <div className="px-3 py-4 border-t border-slate-200/60">
          <button
            ...
            className="group flex items-center gap-3 px-4 py-2.5 text-sm font-semibold text-red-600 rounded-xl hover:bg-red-50 transition-all duration-200"
          >
            <LogOut 
            size={18} 
            strokeWidth={2.5}
            className="transition-transform duration-200 gropu-hover:scale-110" 
            />
            Logout
          </button>
        </div>
